  ...
  <div class="flex flex-col justify-between items-center border-2 border-gray-200 rounded-md">
    ...
    <div class="flex flex-col items-center w-full pb-2 gap-4">
      ...
      <div class="flex flex-col w-11/12 mx-auto gap-3">
        <!-- plan item -->
        <InputRadio class="w-full text-base-content bg-base-200 border-base-200" v-for="plan in content.plans"
          :key="plan.uuid" :id="plan.uuid" v-model="plan.uuid" name="plan" @change="plan_changed(plan.uuid)">
      ...
      <div class="flex flex-col w-full items-center gap-6">
        ...
        <div class="flex flex-col align-center justify-end gap-2 h-14 w-11/12 pb-1">
          <!-- full price -->
          <del :class="(form.plan.price?.value ?? content.plans[0].price.final) !==
        ...
        <form @submit.prevent="check_discount()" class="relative flex flex-row items-center w-11/12 mx-auto"
          :class="{ 'border-error': !discount_error.ok, 'border-b-primary-hover': discount_error.ok }">
          <input type="text" class="w-full rounded-[3px] border-1 border-b-2 text-xs" @focus="text_box_focus(true)"
            @blur="text_box_focus(false)" :class="textbox_border_check(discount_error.ok ?? null)"
